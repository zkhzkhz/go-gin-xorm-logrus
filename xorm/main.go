...
func init() {
	var err error
	engine, err = xorm.NewEngine("mysql", "root:123456@(172.16.16.186:3306)/hlj?charset=utf8")
	_ = engine.Ping()
	if err != nil {
		log.Warn(err)
	}
	...
	//写入文件
	f, err := os.OpenFile("api.log", os.O_WRONLY|os.O_APPEND|os.O_CREATE, 0777)
	if err != nil {
...
func GetHelp(id string) models.KsHelp {
	ksHelp := models.KsHelp{}
	_, err := engine.ID(id).Get(&ksHelp)
	if err != nil {
		log.Warn(err)
	}
	return ksHelp
...
//	engine.Logger().SetLevel(core.LOG_INFO)
//	//写入文件
//	f, err := os.OpenFile("log/api.log", os.O_WRONLY|os.O_APPEND|os.O_CREATE, 0777)
//	if err != nil {
//		logrus.Info(err.Error())
//		return
//	}
